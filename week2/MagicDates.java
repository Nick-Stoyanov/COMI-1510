 ...
 * the year: 6/10/60 Write a program that asks the user to enter a month (in numeric form), a day, and a two-digit year.
 * The program should then determine whether the month times the day is equal to the year. If so, it should display a
...
public class MagicDates
{
	...
	if (monthInput > 0 && monthInput <= 12)
	{
		...
		if (yearInput < 100)
		{
		    ...
		    {
			JOptionPane.showMessageDialog(null, "This is a magic date!");

		    }
		    // Display if result is not a magic date
		    else
			JOptionPane.showMessageDialog(null, "This is not a magic date!");
		}
		// Display if year input is outside of range
		else
		    JOptionPane.showMessageDialog(null,"Invalid input. Enter a number between 0 and 99");
	    }
	    // Display if day input is outside of range
	    else
		JOptionPane.showMessageDialog(null,"Invalid input. Enter a number between 1 and 31.");

	}
	// Display if month input is outside of range
	else
	    JOptionPane.showMessageDialog(null,"Invalid input. Enter a number between 1 and 12.");
